...
const buildOcorrenciaGenerica = (ctx: DocContext, incident: Incident, title: string) => {
  ...
  // CLASSIFICAÇÃO DA OCORRÊNCIA E GRADAÇÃO DA RESPOSTA INSTITUCIONAL (Art. 5º)
  writeSectionTitle(ctx, "CLASSIFICAÇÃO DA OCORRÊNCIA E RESPOSTA INSTITUCIONAL (ART. 5º)");
  let nivelTexto = "";
  if (incident.severity === 'Baixa') {
    nivelTexto = "Nível I – Situação ordinária de indisciplina ou conflito cotidiano. Aplicação das intervenções pedagógicas e dos fluxos previstos no DOC - Documento Orientador para a Convivência - Protocolo 179.";
  } else if (incident.severity === 'Crítica') {
    nivelTexto = "Nível III – Situação de risco, grave ruptura da convivência ou esgotamento das intervenções anteriores. Avaliação da necessidade de afastamento preventivo temporário e, excepcionalmente, de transferência cautelar.";
  } else {
    nivelTexto = "Nível II – Situação recorrente ou complexa, sem risco imediato. Intensificação do acompanhamento pedagógico, articulação com a família, repactuação de combinados, elaboração de plano individual de acompanhamento ou estudo dirigido.";
  }
  writeParagraph(ctx, `Nível de Resposta Aplicado: ${nivelTexto}`, { bold: true });

  // ACOLHIMENTO E ESCUTA QUALIFICADA (Art. 7º, I)
  writeSectionTitle(ctx, "ACOLHIMENTO E PROCEDIMENTOS PROTETIVOS (ART. 7º, I)");
  writeChecklistItem(ctx, true, "Acolhimento inicial e escuta qualificada dos envolvidos realizados (Art. 3º, VIII e IX)");
  writeChecklistItem(ctx, true, "Garantia de oportunidade de manifestação assegurada ao estudante e familiares (Art. 3º, V)");
  writeChecklistItem(ctx, true, "Preservação da dignidade, vedado tratamento vexatório ou discriminatório (Art. 4º, VI)");
  ...
  writeSignatureLines(ctx, ["Assinatura do Professor / Registrante", "Assinatura da Direção / Gestão"]);
...
const buildEncaminhamentoPedagogico = (ctx: DocContext, incident: Incident) => {
  ...
  writeParagraph(
    ...
    "sala de aula e in seu encaminhamento a outro espaço da unidade escolar para realização de atividades pedagógicas " +
  ...
  // GARANTIA DA CONTINUIDADE DA APRENDIZAGEM (Art. 7º, § 4º)
  writeSectionTitle(ctx, "GARANTIAS PEDAGÓGICAS E DE CONVIVÊNCIA (ART. 7º, §§ 4º e 6º)");
  writeChecklistItem(ctx, true, "Garantia de continuidade do processo de aprendizagem e acesso a conteúdos curriculares (Art. 7º, §4º)");
  writeChecklistItem(ctx, true, "Preservação da carga horária e direito à realização de avaliações (Art. 7º, §4º)");
  writeChecklistItem(ctx, true, "Supervisão integral por profissional designado da equipe escolar, vedada a permanência sem atividades (Art. 7º, §6º)");
...
const buildRelatorioCircunstanciado = (ctx: DocContext, incident: Incident) => {
  ...
  writeNumberedItem(ctx, "I - IDENTIFICAÇÃO DA UNIDADE ESCOLAR E EQUIPE RESPONSÁVEL", `Unidade escolar: E.E. PROFª LYDIA KITZ MOREIRA. Equipe responsável pela elaboração: ${incident.professorName || "NÃO INFORMADO"}.`);
  writeNumberedItem(ctx, "II - IDENTIFICAÇÃO DO ESTUDANTE COM PRESERVAÇÃO DE SIGILO", `Estudante identificado(a) nos registros internos sob RA ${incident.ra || "NÃO INFORMADO"}, preservado o sigilo das informações pessoais neste e nos demais documentos do procedimento.`);
  writeNumberedItem(ctx, "III - DESCRIÇÃO OBJETIVA E CRONOLÓGICA DOS FATOS", d?.cronologiaFatos || incident.description);
  writeNumberedItem(ctx, "IV - ANÁLISE DA SITUAÇÃO ATUAL DE RISCO", d?.analiseRisco || "NÃO INFORMADO");
  writeNumberedItem(ctx, "V - REGISTROS NA PLATAFORMA CONVIVA OU EQUIVALENTE", d?.registrosPlataformaConviva || "Registros atualizados na Plataforma Conviva, ou sistema institucional equivalente, ao longo de todo o acompanhamento.");
  writeNumberedItem(ctx, "VI - MEDIDAS PEDAGÓGICAS, RESTAURATIVAS E PROTOCOLARES ADOTADAS", d?.medidasAdotadasAnteriormente || "NÃO INFORMADO");
  writeNumberedItem(ctx, "VII - COMUNICAÇÕES REALIZADAS À FAMÍLIA OU RESPONSÁVEIS", d?.comunicacoesFamilia || "NÃO INFORMADO");
  writeNumberedItem(ctx, "VIII - REGISTROS DE REUNIÕES, ORIENTAÇÕES E PLANOS DE ACOMPANHAMENTO", d?.registrosReunioesPactuacoes || "Sem registros adicionais de reuniões, orientações ou pactuações além dos já mencionados.");
  writeNumberedItem(ctx, "IX - ENCAMINHAMENTOS À REDE PROTETIVA OU ÓRGÃOS COMPETENTES", d?.encaminhamentosRedeProtetiva || "Não houve encaminhamento à rede protetiva ou a outros órgãos competentes neste procedimento.");
  writeNumberedItem(ctx, "X - JUSTIFICATIVA TÉCNICA PARA A EVENTUAL TRANSFERÊNCIA CAUTELAR", d?.justificativaTecnicaTransferencia || "NÃO INFORMADO");
  writeNumberedItem(ctx, "XI - PROPOSTA PRELIMINAR DE CONTINUIDADE DO ACOMPANHAMENTO", d?.propostaContinuidadeAcompanhamento || "A continuidade do acompanhamento pedagógico e protetivo do(a) estudante será definida em conjunto com a URE e a unidade escolar de destino.");
  writeNumberedItem(
    ctx,
    "XII - INFORMAÇÃO SOBRE MATRÍCULA DECORRENTE DE DECISÃO JUDICIAL",
...
const addFooters = (ctx: DocContext) => {
  const { doc, pageWidth, pageHeight } = ctx;

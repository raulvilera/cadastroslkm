...
interface DashboardProps {
  ...
  professorIncidentsCount?: number;
  managementIncidentsCount?: number;
  students: Student[];
  classes: string[];
  ...
  onLoadIncidentsByStatus?: (status: string) => Promise<Incident[]>;
  onToggleView?: () => void;
  viewMode?: 'gestor' | 'professor';
}

const Dashboard: React.FC<DashboardProps> = ({ user, incidents, totalIncidentsCount, professorIncidentsCount, managementIncidentsCount, students, classes, onSave, onDelete, onLogout, onOpenSearch, onUpdateIncident, onSyncStudents, onLoadFullStudentHistory, onLoadArchivedIncidents, onLoadIncidentsByStatus, onToggleView, viewMode }) => {
  ...
  // Registros do status selecionado buscados no banco INTEIRO (sem o limite de
  // 30 dias do carregamento padrão) — assim, ao filtrar por "Pendente",
  // "Em andamento", "Resolvida" ou "Visualizada", o painel mostra TODAS as
  // ocorrências daquele status, não apenas as dos últimos 30 dias.
  const [statusFilteredIncidents, setStatusFilteredIncidents] = useState<Incident[] | null>(null);
  const [isLoadingStatusFilter, setIsLoadingStatusFilter] = useState(false);

  useEffect(() => {
    if (statusFilter.toLowerCase() === 'todos') {
      setStatusFilteredIncidents(null);
      return;
    }
    if (!onLoadIncidentsByStatus) return;

    let cancelled = false;
    setIsLoadingStatusFilter(true);
    onLoadIncidentsByStatus(statusFilter)
      .then(data => { if (!cancelled) setStatusFilteredIncidents(data); })
      .catch(() => { if (!cancelled) setStatusFilteredIncidents(null); })
      .finally(() => { if (!cancelled) setIsLoadingStatusFilter(false); });

    return () => { cancelled = true; };
  }, [statusFilter, onLoadIncidentsByStatus]);
  ...
  const history = useMemo(() => {
    ...
    // Quando um status específico está selecionado, usa o resultado buscado no
    // banco INTEIRO (statusFilteredIncidents); caso contrário, usa a lista
    // padrão dos últimos 30 dias (incidents).
    const baseList = statusFilter.toLowerCase() !== 'todos' && statusFilteredIncidents
  ...
  }, [incidents, statusFilteredIncidents, searchTerm, statusFilter]);
  ...
  // Mesma lógica do total geral, mas separada por origem do registro (professor x gestão):
  // quando não há filtro/busca ativos, exibe o total REAL (banco todo); caso contrário,
  // exibe a contagem dos registros atualmente carregados/filtrados que batem com cada origem.
  const noFilterActive = statusFilter.toLowerCase() === 'todos' && !searchTerm.trim();
  const displayedProfessorCount = (noFilterActive && typeof professorIncidentsCount === 'number')
    ? professorIncidentsCount
    : history.filter(i => i.source === 'professor').length;
  const displayedManagementCount = (noFilterActive && typeof managementIncidentsCount === 'number')
    ? managementIncidentsCount
    : history.filter(i => i.source === 'gestao').length;
      ...
      <main className="max-w-[1700px] mx-auto mt-6 sm:mt-8 px-4 sm:px-6 space-y-8 sm:space-y-10">
        {activeTab === 'registros' && (
          <>
              ...
              <div className="p-6 sm:p-10 bg-gradient-to-br from-blue-800 via-blue-900 to-blue-950">
                <form onSubmit={handleSave} className="space-y-6 sm:space-y-8">
                  <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-end">
                    <div className="flex flex-col gap-2 w-full lg:w-64">
                      ...
                      <StripedSelect
                        ...
                        triggerClassName="h-12 sm:h-14 border-2 border-white rounded-2xl px-5 text-base font-bold !text-white bg-green-900 focus:ring-2 focus:ring-blue-500 outline-none shadow-sm cursor-pointer w-full"
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="w-full h-[550px] overflow-y-auto bg-green-900 border-2 border-white rounded-2xl p-6 custom-scrollbar grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 shadow-inner">
                        ...
                        return (
                          <label key={a.ra || idx} className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all cursor-pointer relative group
                            ${selected
                              ...
                              : 'card-glow-idle bg-gradient-to-br from-[#1a336b] to-black border-white/10 text-white/70 hover:from-blue-800 hover:to-black hover:border-white/30'}`}
                    ...
                    <div className="bg-white/10 rounded-3xl p-6 space-y-5 border border-white/20 animate-fade-in">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                          ...
                          const selected = epAtividades.includes(opt.key);
                          return (
                            <label
                              key={opt.key}
                              className={`flex items-center gap-2 text-[10px] font-bold uppercase rounded-xl px-4 py-3 cursor-pointer border-2 transition-all duration-200 ${
                                selected
                                  ? 'bg-teal-500/90 border-teal-300 text-white shadow-[0_4px_14px_rgba(20,184,166,0.5)] scale-[1.02]'
                  ...
                  {classification === 'AFASTAMENTO PREVENTIVO TEMPORÁRIO' && (
                    <div className="bg-white/10 rounded-3xl p-6 space-y-5 border border-white/20 animate-fade-in">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {[
                          { checked: apComunicadoFamilia, onChange: setApComunicadoFamilia, label: 'Comunicado à família/responsáveis' },
                          { checked: apComunicadoURE, onChange: setApComunicadoURE, label: 'Comunicado à URE' },
                          { checked: apComunicadoRedeProtetiva, onChange: setApComunicadoRedeProtetiva, label: 'Comunicado à rede protetiva' },
                          { checked: apOportunidadeManifestacao, onChange: setApOportunidadeManifestacao, label: 'Oportunidade de manifestação assegurada (Art. 12, § 3º)' },
                        ].map((item, idx) => (
                          <label
                            key={idx}
                            className={`flex items-center gap-2 text-[10px] font-bold uppercase rounded-xl px-4 py-3 cursor-pointer border-2 transition-all duration-200 ${
                              item.checked
                                ? 'bg-[#6b8e23]/90 border-white text-white shadow-[0_4px_10px_rgba(0,0,0,0.45)] scale-[1.02]'
                                : 'bg-white/10 border-transparent text-white hover:bg-white/15'
                            }`}
                          >
                            <input
            ...
            <section className="bg-transparent rounded-[32px] shadow-2xl border border-gray-100 mt-10 max-h-[700px] overflow-y-auto custom-scrollbar relative">
              <div className="px-6 sm:px-10 py-6 bg-gradient-to-r from-black via-blue-900 to-black text-white flex flex-col md:flex-row justify-between items-center gap-4 sticky top-0 z-30 shadow-[0_15px_30px_rgba(0,0,0,0.3)] rounded-t-[32px]">
                ...
                <div className="flex-1 flex flex-col sm:flex-row items-center justify-center gap-3 order-first md:order-none z-10 w-full md:w-auto">
                  <div className="w-full sm:w-auto bg-white/5 backdrop-blur-xl border border-white/10 px-4 sm:px-8 py-3 rounded-[24px] flex items-center justify-center gap-3 sm:gap-5 group hover:bg-white/10 transition-all duration-500 shadow-[0_8px_32px_rgba(0,0,0,0.3)] hover:scale-105 border-b-2 border-b-blue-500/50 flex-wrap">
                    ...
                    <div className="flex flex-col items-center shrink-0">
                      <span className="text-[9px] font-black text-blue-200/70 uppercase tracking-[0.25em] leading-none mb-1.5 drop-shadow-sm whitespace-nowrap">Total de Ocorrências</span>
                    ...
                    {/* Pequenos círculos: total por origem (Professores x Gestão) */}
                    <div className="flex items-center gap-3 pl-3 sm:pl-4 border-l border-white/10 shrink-0">
                      <div className="flex flex-col items-center gap-1" title="Total de ocorrências registradas por professores">
                        <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-emerald-400/20 to-emerald-600/20 border border-emerald-400/40 flex items-center justify-center shadow-[0_0_12px_rgba(16,185,129,0.35)]">
                          <span className="text-emerald-300 text-[11px] sm:text-xs font-black leading-none">
                            {displayedProfessorCount < 10 ? `0${displayedProfessorCount}` : displayedProfessorCount}
                          </span>
                        </div>
                        <span className="text-[7px] sm:text-[8px] font-black text-emerald-300/70 uppercase tracking-widest leading-none">Prof.</span>
                      </div>
                      <div className="flex flex-col items-center gap-1" title="Total de ocorrências registradas pela gestão">
                        <div className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-purple-400/20 to-purple-600/20 border border-purple-400/40 flex items-center justify-center shadow-[0_0_12px_rgba(168,85,247,0.35)]">
                          <span className="text-purple-300 text-[11px] sm:text-xs font-black leading-none">
                            {displayedManagementCount < 10 ? `0${displayedManagementCount}` : displayedManagementCount}
                          </span>
                        </div>
                        <span className="text-[7px] sm:text-[8px] font-black text-purple-300/70 uppercase tracking-widest leading-none">Gestão</span>
                      </div>
                    </div>
                  </div>

                  {/* Dropdown de Filtro de Status (NOVO) */}
                  <div className="relative group w-full sm:w-auto sm:min-w-[140px]">
              ...
              <div className="bg-white rounded-b-[32px] overflow-hidden">
                ...
                <div className="sm:hidden flex flex-col gap-[12px] bg-gray-100/80 p-3">
                  {isLoadingStatusFilter ? (
                    <div className="p-20 text-center text-gray-400 font-black uppercase text-xs tracking-widest">Carregando todo o histórico deste status...</div>
                    ...
                    <div className="p-20 text-center text-gray-300 font-black uppercase text-xs tracking-widest">Nenhum registro encontrado</div>
                  )}
                </div>
                ...
                <table className="hidden sm:table w-full text-left text-[10px] min-w-[1200px]">
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    {isLoadingStatusFilter ? (
                      <tr><td colSpan={9} className="p-20 text-center text-gray-400 font-black uppercase text-xs tracking-widest">Carregando todo o histórico deste status...</td></tr>
                    ) : history.length > 0 ? history.map(inc => (
                      ...
                      <tr><td colSpan={9} className="p-20 text-center text-gray-300 font-black uppercase text-xs tracking-widest">Nenhum registro encontrado</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 items-end">
                ...
                <div className="w-full sm:w-44 space-y-1">
                  ...
                  <StripedSelect
                    ...
                    onChange={setArchiveSearchClass}
                    options={classes}
                    placeholder="Todas as turmas"
                    triggerClassName="w-full h-11 px-4 bg-white border border-gray-200 rounded-2xl text-base font-bold outline-none focus:ring-2 focus:ring-purple-500 transition-all text-black cursor-pointer"
                    optionTextClassName="text-base"
                    centered
                  />
                </div>

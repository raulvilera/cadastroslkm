  ...
  const referralLabels = (inc: Incident): string[] => {
    ...
    if (refs.length > 0) {
      return refs.map(r =>
        ...
        r.type === 'acidente'              ? '🚨 Acidente' : r.type
      );
    }
    // Legado
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-800 via-blue-900 to-blue-950 font-sans pb-12" style={{ paddingTop: headerHeight }}>
      ...
      <main className="max-w-7xl mx-auto mt-8 px-6 space-y-8">
        {/* FORMULÁRIO */}
        <div className="bg-gradient-to-br from-blue-800 via-blue-900 to-blue-950 rounded-xl shadow-2xl overflow-hidden border border-white/10">
          <div className="bg-gradient-to-r from-black to-blue-900 py-3 text-center border-b border-blue-700/40">
        ...
        {/* PAINEL DE REGISTROS */}
        <section className="bg-transparent rounded-xl shadow-2xl border border-gray-300 max-h-[700px] overflow-y-auto custom-scrollbar relative">
          <div className="px-6 sm:px-8 py-5 border-b border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 bg-gradient-to-r from-black via-blue-900 to-black shadow-[0_15px_30px_rgba(0,0,0,0.3)] sticky top-0 z-30 rounded-t-xl">
                    ...
                    {referralLabels(inc).length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {referralLabels(inc).map((lb, i) => (
                          <span key={i} className={`text-[8px] font-bold px-1.5 py-0.5 rounded ${lb.includes('Orientação') ? 'bg-emerald-100 text-emerald-700' : lb.includes('Gestora') ? 'bg-orange-100 text-orange-700' : lb.includes('Incidente') ? 'bg-yellow-100 text-yellow-700' : lb.includes('Acidente') ? 'bg-red-100 text-red-700' : 'bg-sky-100 text-sky-700'}`}>{lb}</span>
                        ))}
                      </div>
                    )}
                    {/* Linha 5: descrição */}
                  ...
                  <tr key={inc.id} className={`transition-colors ${unread ? 'bg-amber-50 border-l-4 border-l-amber-400 hover:bg-amber-100' : hasFb ? 'bg-emerald-50/30 hover:bg-emerald-50/60' : 'hover:bg-blue-50/50'}`}>
                    ...
                    <td className="border border-gray-300 px-3 py-2 text-[9px] font-bold max-w-[160px]">
                      {referralLabels(inc).length > 0 ? referralLabels(inc).map((lb, i) => (
                        <span key={i} className={`block text-[9px] font-bold ${lb.includes('Orientação') ? 'text-emerald-600' : lb.includes('Gestora') ? 'text-orange-600' : lb.includes('Busca') ? 'text-sky-600' : lb.includes('Incidente') ? 'text-yellow-600' : lb.includes('Acidente') ? 'text-red-600' : 'text-gray-400'}`}>{lb}</span>
